fwd_select = function(data, ind_vars, nboot, seed.start){
  library("pROC")
  inc_vars = c()
  ...
  N = nrow(data)
  dat_boot = data[,c("yr5_mort","wt4yr_norm",ind_vars)]
  for(i in 1:length(ind_vars)){
    ...
    for(j in 1:nboot){
      ...
      for(k in 1:length(exc_vars)){
        ...
        auc_ij[j,k] = roc(dat_tmp_pred$yr5_mort,predict(fit_tmp, newdata=dat_tmp_pred,type='response'))$auc[1]
