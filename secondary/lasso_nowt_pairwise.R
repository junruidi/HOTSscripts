...
y_keep = c("yr5_mort","Diabetes","Cancer","CHF","CHD")
x_keep_all = c(paste0("sc2_",c(1:12)),paste0("hosvd_sc3_",c(1:12)),paste0("hosvd_sc4_",c(1:12)),
               paste0("rob_sc3_",c(1:12)),paste0("rob_sc4_",c(1:12)))
y_vars = survival_all[,y_keep]
survival_all = survival_all[,x_keep_all]
survival_all = as.data.frame(scale(survival_all, center = T, scale = T))
survival_all = cbind(y_vars, survival_all)
rm(list = setdiff(ls(),"survival_all"))
...
x_keep1 = paste0("sc2_",c(1:12))
x_keep2 = c(paste0("sc2_",c(1:12)),paste0("hosvd_sc3_",c(1:12)),paste0("hosvd_sc4_",c(1:12)))
x_keep3 = c(paste0("sc2_",c(1:12)),paste0("rob_sc3_",c(1:12)),paste0("rob_sc4_",c(1:12)))
...
insamp1 = model.matrix(~ .*., survival_all[,x_keep1])[,-1]
insamp2 = model.matrix(~ .*., survival_all[,x_keep2])[,-1]
insamp3 = model.matrix(~ .*., survival_all[,x_keep3])[,-1]


train1 = model.matrix(~ .*.,train[,x_keep1])[,-1]
train2 = model.matrix(~ .*.,train[,x_keep2])[,-1]
train3 = model.matrix(~ .*.,train[,x_keep3])[,-1]

test1 = model.matrix(~ .*.,test[,x_keep1])[,-1]
test2 = model.matrix(~ .*.,test[,x_keep2])[,-1]
test3 = model.matrix(~ .*.,test[,x_keep3])[,-1]
...
fit_yi_x1_cv = repeated.cv.glmnet(x = train1, y = y_train, n = 1)
...
fit_yi_x1_cv_is = repeated.cv.glmnet(x = insamp1, y = y_all, n = 1)
...
fit_yi_x2_cv = repeated.cv.glmnet(x = train2, y = y_train, n = 1)
...
fit_yi_x2_cv_is = repeated.cv.glmnet(x = insamp2, y = y_all, n = 1)
...
fit_yi_x3_cv = repeated.cv.glmnet(x = train3, y = y_train, n = 1)
...
fit_yi_x3_cv_is = repeated.cv.glmnet(x = insamp3, y = y_all, n = 1)
...
# I3_h234 = coef_x2
# I3_r234 = coef_x3
# save(I3_2, I3_h234, I3_r234, file = "I3.rda")
